...
//require()函数在真正加载模块之前，会先去package.loaded表（这个表在全局环境里）里按名字查找模块
//标准库全部都是模块的形式提供。Lua发布版携带的解析器已经默认打开了全部标准库，因此这些库都在package.loaded表里
//require()函数先求助搜索器（Searcher），搜索器是一个普通函数，接受唯一参数是模块名。
//搜索器可以有多个，按顺序存储在package.searchers数组里（这个表在全局环境里）。Lua官方实现预定义了四个搜索器，按顺序分别是preload搜索器、Lua搜索器、C搜索器和CRoot搜索器
//preload搜索器只是简单的搜索package.preload表
//Lua搜索器从package.path变量（这个变量在全局环境里）指定的路径找Lua文件，然后加载该文件，加载后的函数就是加载器。
//文件目录分隔符、搜索路径分隔符、模块名占位符（默认是问号）等记录在package.config变量里（这个变量在全局环境里）
...
func OpenPackageLib(ls LuaState) int {
	...
	/* set field 'loaded' */
	ls.GetSubTable(LUA_REGISTRYINDEX, LUA_LOADED_TABLE)		//把package.loaded放进注册表
	ls.SetField(-2, "loaded")
	/* set field 'preload' */
	ls.GetSubTable(LUA_REGISTRYINDEX, LUA_PRELOAD_TABLE)	//把package.preload放进注册表
