...
//基础库是通过全局变量的形式提供的。
//基础库一共提供了24个全局变量。
//其中_VERSION是字符串类型变量，表示Lua的版本号。_G是表类型的变量，全局变量实际上是某个表的字段，这个表就是_G。
//剩下的22个全局变量是函数类型，后面称它们为全局函数。这22个全局函数大致上又分为类型相关、错误处理相关、迭代器相关、元编程相关、加载等六类。
...
func baseSelect(ls LuaState) int {
	...
	if ls.Type(1) == LUA_TSTRING && ls.CheckString(1) == "#" {
		...
		i := ls.CheckInteger(1)	//第一个参数必须是整数
		if i < 0 {
			i = n + i
		} else if i > n {
			i = n
