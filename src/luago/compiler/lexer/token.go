...
// token 概念：
// 我们在阅读英文时，大脑并不是以字母为单位，而是以单词为单位来处理信息的。与此类似，编译器在编译源代码时，也不是以字符为单位，而是以“token”为单位进行处理的。
// 词法分析器的作用就是根据编程语言的词法规则，把源代码（字符流）分解为token流。
// token按其作用可以分为不同的类型，比较常见的类型：空白字符、注释、标识符/关键词、字面量（数字、字符串）、运算符、分隔符等。
